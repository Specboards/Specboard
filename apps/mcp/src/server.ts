...
function db(): Database {
  if (!dbInstance) {
    ...
    if (!url) {
      throw new Error(
        "DATABASE_URL is not set. Point it at the Specboard Postgres (e.g. postgres://postgres:postgres@localhost:5432/specboard) and seed it with `pnpm --filter @specboards/db seed`.",
      );
    }
