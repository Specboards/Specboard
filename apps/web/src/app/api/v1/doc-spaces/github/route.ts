...
/**
 * POST /api/v1/doc-spaces/github: create a private docs repository in the
 * workspace's organization installation and bind it as the area's doc source.
 * Body: { productId, area, name, installationId? }. Mirrors the spec-repo
 * create flow (admin-only, org installations only), but the row is written
 * with `isSpecRepo: false` and no config so spec sync never touches it.
 */
export async function POST(req: Request) {
  ...
  if (!membership || membership.role !== "admin") {
    return Response.json(
      { error: "Only an admin can create repositories." },
      { status: 403 },
    );
  }
  ...
  // Register the repo. `isSpecRepo: false` + no config keeps it out of spec
  // sync; upsert so retrying after a partial failure converges.
  const [repo] = await db
    .insert(repositories)
    .values({
  ...
  if (!repo) {
    return Response.json(
      { error: `Created ${created.owner}/${created.name} on GitHub but couldn't connect it.` },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    const space = await store.setDocSpace(
      ...
      { userId: user.id, workspaceId: membership.workspaceId },
    );
    return Response.json(
      {
        space,
